...
// ──────────────────────────────────────────────
router.post('/chat/stream', optionalAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    // Đã fix: Trả về kiểu number để khớp với ai.service.ts
...
// ──────────────────────────────────────────────
router.get('/history/:session_id', optionalAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
...
// ──────────────────────────────────────────────
router.get('/leads', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const token = readBearerToken(req.headers.authorization);
        if (!token) {
...
// ──────────────────────────────────────────────
router.patch('/leads/:id/review', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const token = readBearerToken(req.headers.authorization);
        if (!token) {
            res.status(401).json({ success: false, message: 'Thiếu access token' });
        ...
        // Đã fix: Ép kiểu param id về dạng number theo yêu cầu của hàm
        ...
        await markLeadReviewed(id, Number(user.id));
...
// ──────────────────────────────────────────────
router.get('/admins/online', async (req: Request, res: Response) => {
    try {
...
// ──────────────────────────────────────────────
router.post('/embed/query', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        // Đã fix: Đổi số 3 thành chuỗi "3" cho khớp yêu cầu hàm
